...
/*********************************************************************************************
* Class: Agent_Msg
* Function: create(Agent *a, Agent_AID aid, Task_FuncPtr behaviour);
* Return type: Int
* Comment: request the Agent to create an Agent, only can be done if Handle
*          is NULL and object.aid is NULL. Returns NO_ERROR
*          if request was posted correctly.
**********************************************************************************************/
//    int Agent_Msg::create(Task_FuncPtr behaviour, Agent_AID &aid){
//        Agent_AID AMS;
//        Agent *agent_caller;
//        agent_caller = (Agent*) Task_getEnv(caller);
//
//        if (a->agent.aid==NULL && aid==NULL){
//            if (agent_caller->agent.org==NULL || (agent_caller->agent.org!=NULL && (agent_caller->agent.role==OWNER || agent_caller->agent.role==ADMIN))){
//
//                msg.type=REQUEST;
//                msg.content_int=CREATE;
//                msg.content_string= (String) behaviour; //Set the content in string variable.
//                msg.target_agent= (Agent_AID) a;
//                msg.sender_agent=Task_self();
//
//                //Get the AMS info*/
//                AMS=agent_caller->agent.AP;
